...
export class LevelCalculatorService {
  ...
  constructor(private storage: DataStoreService) {
    var goalData = this.storage.getItem('goals');
  ...
  getLevelData() {
    ...
    return { currentLevel: this.currentLevel, maxLevel: this.maxLevel };
  }
  ...
  calculateLevelData(data) {
    ...
    data.goals.forEach(goal => {
      ...
      goal.subGoalsData.forEach(subgoal => {
        goalCount++;
    ...
    if(goalCount < 100) {
      this.maxLevel = goalCount;
      subdivisions.push(this.maxLevel);
    } else {
      ...
      for(var i = 0; i < subdivisions.length; i++) {
        this.maxLevel += subdivisions[i]/(i+1);
      }
    }
    ...
    for(var i = 0; i < subdivisions.length && tempCompletedGoalsCount > 0; i++) {
      if(subdivisions[i] < tempCompletedGoalsCount) {
        ...
        this.currentLevel += Math.floor(tempCompletedGoalsCount/(i+1));
